...
public class Calculator {
    ...
    public static void addOperation(String sign, MathOperation mathOperation) {
        map.put(sign, mathOperation);
    }

    public static int calculate(String expression) {
        final String[] exps = expression.split(" ");
        int onScreenNumber = Integer.parseInt(exps[0]);
        for (int i = 1; i < exps.length; i = i + 2) {
            final MathOperation op = map.get(exps[i]);
            final int enteredNumber = Integer.parseInt(exps[i+1]);
